...
UPDATE `articles` 
SET `date_creation`=date_creation-14 # dit is geen veilige manier, bv als date_creation == 10.. -4 als resultaat!!
WHERE name="artikelke1"
# Toon nu enkel die records waarbij het artikel moet aangemaakt zijn in de laatste 7 dagen
SELECT * FROM articles 
WHERE date_creation >= CURDATE()-7 # ook zelfde probleem hier, niet de correcte manier van werken
...
FROM country 
WHERE continent="Africa" AND lifeexpectancy <= 60
ORDER BY country # je kan geen order doen op iets dat niet weergegeven wordt, daarom dus een alias aanmaken
				 # of 1 gebruiken kan ook
# Geef de citycode (=eerste 3 karakters) van elke stad waarvan het land geregeerd wordt door Elisabeth II
SELECT LEFT(city.name,3) AS stadscode 
FROM city 
...
FROM city 
GROUP BY name 
HAVING LENGTH(name) > 20 # WHERE zal wel werken, LENGTH is geen aggregate functie
						 # maar toch best consistent zijn in gebruik
						 # best dus altijd HAVING gebruiken!
# Toon de naam van de landen waar Elisabeth II regeert. 
	# Toon ook in elke record Her Majesty The Queen Elisabeth II
SELECT name, CONCAT('Her Majesty The Queen ',headofstate) 
FROM country
WHERE headofstate="Elisabeth II"
# Toon de gemiddelde levensverwachting in de wereld. Rond af tot 2 cijfers na de komma
SELECT ROUND(AVG(lifeexpectancy), 2) AS life_exp_world FROM country
# Toon een willekeurig land en de huidige systeemdatum&tijd in formaat DD-MM-YYYY HH:MM:SS
SELECT name, DATE_FORMAT(NOW(),'%d-%m-%Y %T') AS cur_date 
FROM country
ORDER BY RAND() LIMIT 1
# Toon een willekeurig artikel uit uw webshop (formaat datum creatie en laatst gewijzigd = dd/mm/’yy
SELECT name, DATE_FORMAT(date_creation, '%d/%m/\'%y') AS was_made, 
DATE_FORMAT(date_last_modified, '%d/%m/\'%y') AS was_last_mod 
FROM articles 
ORDER BY RAND() LIMIT 1

# Datums vergelijken EXTRA:
DATE_ADD()
DATE_SUB()
DATEDIFF(datum1, datum2)
vb.
	DATEDIFF(CURDATE(), date_creation) <= 7
	DATEDIFF()